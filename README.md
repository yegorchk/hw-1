...
Вывод: сильного дисбаланса нет, значит можно спокойно смотреть `accuracy`, но дополнительно использовать `macro-F1`, потому что классов 7.

### Пропуски
Пропусков нет.  
Вывод: замена пропусков не нужна.
...
Вывод: сильной мультиколлинеарности нет, линейные модели не должны портиться из-за дублирующихся числовых признаков.
...
Проверка: Logistic Regression, сравнил без BMI и с BMI.
...
Проверка: DecisionTree с разными `max_depth`.
...
Проверка: сравнил DecisionTree(depth=7) vs RandomForest(300).
...
Проверка: LogReg(+BMI) без весов и с `class_weight='balanced'`.
